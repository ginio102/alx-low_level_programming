 ...
 * @e_ident: array of identifying elf files
 * Failure: If the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: array of Elf files with magic numbers
 * Description: magic numbers if elf file
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: file identifier of elf
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 * @elf: file descriptor
 * if failure: If the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
...
/**
 * main - Displays information in an elf file
 * @argc: the number of arguments
 * @argv: array of arguments.
 * Return: 0 on success
 * Failure: If file is not an ELF File
